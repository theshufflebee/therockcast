...
plot(merge(as.zoo(data$rate), as.zoo(data$inflation)), plot.type="single", col=c(2, 4), lwd=2, main="Rate vs. Inflation"); legend("topleft", c("Rate", "Inflation"), col=c(2, 4), lwd=2, bty="n")
...
```{r Extracting Forecast Data}

#Marcos: This chunk should probably go at the end of the estimation chunk
...
#Since every list in the worker:results list is the forecast at one moment between R and T, we extract each on its own
...
# Spaghetti plot
ggplot(df_all, aes(x = date_of_forecast, y = .data[[model]], group = period)) +
...
ggplot(df_all, aes(x = date_of_forecast, group = period)) +
...
var_by_horizon <- df_all_3 %>%
  ...
  summarize(
    ...
    var_fe = sd(forecast_error, na.rm=T)^2,
    n = n() )
