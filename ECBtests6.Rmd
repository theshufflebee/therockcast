...
generate_mincer_zarnowitz_report <- function(F_model, 
  ...
  for (h in 1:H) {
    ...
    if (nrow(df_h) > 2) {
      ...
      test_joint <- linearHypothesis(mz_reg, c("(Intercept) = 0", "forecasts = 1"))
  ...
  table_output <- kable(
    ...
    digits = 4,
    col.names = c("h", "Alpha", "Beta", "p(Joint)"),
    escape = FALSE ) %>%
    kable_styling(
    ...
    footnote(
      general = "p(Joint) is the p-value for the joint hypothesis H_0: (Alpha, Beta) = (0, 1). A high p-value means we fail to reject the null hypothesis of an unbiased, efficient forecast.",
      symbol = c(
...
worker_results <- foreach(
  ...
  # We no longer calculate errors inside the loop
