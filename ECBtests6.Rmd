...
require(strucchange) #Chow test for structural breaks
...
```{r data_config, message=TRUE, warning=FALSE}
...
if (USE_INFLATION_EXPECTATIONS) {
  ...
  message(">> CONFIGURATION: Using inflation expectations in Taylor Rule forecasting.")
} else {
  data$inflation_gap <- data$realised_inflation_gap 
  message(">> CONFIGURATION: Using realised inflation in Taylor Rule forecasting.") }
...
ggplot(data, aes(x = date, color = series)) +
  ...
  geom_line(aes(y = inflation, color = "(Realised) Inflation"), linewidth = 1) +
  geom_line(aes(y = exp_inflation, color = "(Expected) Inflation"), linewidth = 1) +
  geom_line(aes(y = output_gap_hp, color = "Output Gap (HP)"), linewidth = 1) +
  geom_line(aes(y = output_gap_ham, color = "Output Gap (Hamilton)"), linewidth = 1) +
  labs(title = "Raw Data Plots", x = "", y = "", color = "Time Series") +
  theme_minimal()
...
chow_test1 <- sctest(rate ~ rate_lag + inflation_gap + output_gap, type = "Chow", point = breakpoint1, data = data)

chow_test2 <- sctest(rate ~ rate_lag + inflation_gap + output_gap, type = "Chow", point = breakpoint2, data = data)
...
Prediction intervall pseudo code

get sd of forecast errors
define alpha = 0.05
(1-alpha)/0.6827 = mult

sd*mult +_ point forecast are the prediction intervalls

store upper and lower 1-alpha in seperate columns and give identifying name so we can make a 50% and 95% forecast


```{r prediction_intervalls, echo=TRUE, message=FALSE, warning=FALSE, results='asis'}
prediction <- var_by_horizon
prediction$sd_fe <- sqrt(prediction$var_fe)

final_intervall$sd <- prediction$sd_fe
final_intervall$upper_1_sd <- final_intervall$sd + final_intervall$TR_Forecast
final_intervall$lower_1_sd <- final_intervall$sd*(-1) + final_intervall$TR_Forecast

final_intervall$upper_2_sd <- final_intervall$sd*2 + final_intervall$TR_Forecast
final_intervall$lower_2_sd <- final_intervall$sd*2*(-1) + final_intervall$TR_Forecast

final_intervall <- as.data.frame(final_intervall)
```


```{r prediction_intervalls_plot, echo=TRUE, message=FALSE, warning=FALSE, results='asis'}
ggplot(final_intervall, aes(x = seq_len(nrow(final_intervall)), y = TR_Forecast, group = 1)) +
  ...
  labs(title = "Prelinimary Forecast plot", x = "h step ahead") +
  theme_minimal() +
