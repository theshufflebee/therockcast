...
I can ask Marco Sotonovoa for questions on the code but dont tell prof MDR
...
# MRR (communication) (just to test)
ecb_mrr_daily <- fredr(series_id = "ECBMRRFR", observation_start = as.Date(start_date))
ecb_mrr_q <- ecb_mrr_daily %>%
  mutate(quarter = as.yearqtr(date)) %>%
  group_by(quarter) %>%
  summarise(rate = last(value)) %>%
  mutate(date = as.Date(quarter))
colnames(ecb_mrr_q)[which(names(ecb_mrr_q) == "rate")] <- "MRR"
...
worker_results <- foreach(
  ...
  TR_preds_1 <- pmax(predict(TR_model_1, new_data_base), -0.5)
  TR_preds_2 <- pmax(predict(TR_model_2, new_data_base), -0.5)
  TR_preds_3 <- pmax(predict(TR_model_3, new_data_3), -0.5)
  TR_preds_4 <- pmax(predict(TR_model_4, new_data_4), -0.5)
  BM_preds   <- pmax(BMpredicted_rates, -0.5) # Benchmark
