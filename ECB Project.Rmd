...
source(here("helpers/jarque_bera_test_helpers.R"))
...
plot_data <- data %>%
  pivot_longer(cols = c(rate, inflation, exp_inflation, 
...
# Tables
stat_table <- kable(all_stationarity_results, digits = 4, 
      format = format, booktabs = TRUE, 
...
chow_test1 <- sctest(break_formula, type = "Chow", 
                     point = breakpoint1_obs, data = data)
chow_test2 <- sctest(break_formula, type = "Chow", 
                     point = breakpoint2_obs, data = data)

# Table with Chow results (for suspected breaks)
chow_df <- data.frame(Event = c("ZLB Start", "COVID-19 Start"),
...
cat("Evaluation sample starts after ",
    as.character(data$quarter[R]),".",sep="")
...
worker_results <- foreach(
  ...
  interest_arma <- my.auto.arima(training$rate, max.p=4, max.q=4, d=1)
  ...
  TR_preds_1 <- round(pmax(predict(TR_model_1, new_data_base), 
                           min(data$rate)) / 0.25) * 0.25
  TR_preds_2 <- round(pmax(predict(TR_model_2, new_data_base), 
                           min(data$rate)) / 0.25) * 0.25
  BM_preds <- round(pmax(BMpredicted_rates, 
                           min(data$rate)) / 0.25) * 0.25
...
forecast_to_df <- function(forecast_list, period) {
  numeric_list <- lapply(forecast_list, function(x) as.numeric(x)) 
...
spag_plot1 <- ggplot(eval_all_models, 
                     aes(x = date_of_forecast, 
                         y = .data[[model]], 
                         group = period, color = factor(period))) +
  geom_line(alpha = 0.7) +    
...
spag_plot2 <- ggplot(eval_all_models, 
                     aes(x = date_of_forecast, 
                         group = period)) +
...
plot_facet1 <- ggplot(eval_all_models %>% filter(horizon <= H), 
...
plot_facet2 <- ggplot(eval_all_models %>% filter(horizon <= H), 
                      aes(x = forecast_error)) +
  geom_density(fill = "#3498db", color = "#2980b9", alpha = 0.6) +
...
our_predict <- function(data,formula,H){
  ...
  # --- 1. Fit inputs and benchmark models  ---
   inflation_arma <- my.auto.arima(data$inflation_gap, max.p=4, max.q=4, 
                                   d=1, var_name="Inflation Gap")
   outputgap_arma <- my.auto.arima(data$output_gap, max.p=4, max.q=4, 
                                   d=0, var_name="Output Gap")
   interest_arma <- my.auto.arima(data$rate, max.p=4, max.q=4, 
                                  d=1, var_name="Interest Rate")
...
### Prepare Data
```{r pred_int_data, echo=TRUE, message=FALSE, warning=FALSE, results='asis'}
...
```{r pred_int_plot, echo=TRUE, message=FALSE, warning=FALSE, results='asis'}
