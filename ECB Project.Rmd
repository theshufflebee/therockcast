...
* Potential GDP: Either estimated with the Hodrick-Prescott filter, or the Hamilton filter, based on the formulae below:
    $$
      \begin{aligned}
...
* Inflation:
  + Realised: The Inflation data is from Eurostat and called HICP. It's the 
    index the ECB uses for Inflation and is not seasonally adjusted.
...
Pseudo-out of sample recursive estimation scheme of direct forecasts for all
...
final_fc_plot2 <- ggplot(final_interval, aes(x = seq_len(nrow(final_interval)), y = TR_Forecast, group = 1)) +
  ...
  # --- 2. Forecast Line and Points ---
  # Forecast Line (Center)
  ...
  # --- 3. Labels and Theme ---
  labs(
    title = "Monetary Policy Rate Forecast with Uncertainty Bands",
    ...
    x = "Forecast Horizon (h-steps ahead)",
    y = "Interest Rate (%)"
  ) +
  
  # --- 4. Scale and Aesthetics ---
  # Add a horizontal line at 0 or a key policy rate for reference
  # geom_hline(yintercept = 0, linetype = "dashed", color = "red", alpha = 0.7) +
  
  theme_light() + # Use a cleaner, professional theme
  theme(
    plot.title = element_text(face = "bold", size = 16, hjust = 0.5, margin = margin(b=7)),
    ...
    panel.grid.minor.x = element_blank(),
    axis.text.x = element_text(angle = 0, hjust = 0.5) # Resetting angle for cleaner look
  )
...
```{r Durbin_Watson_Test, echo=TRUE, message=FALSE, warning=FALSE, results='asis'}
...
for (model_name in formula_cols) {
  ...
  all_forecast_errors <- eval_all_models[["actuals"]] - eval_all_models[[model_name]]
...
```{r Ljung_Box_Test, echo=TRUE, message=FALSE, warning=FALSE, results='asis'}
#Ljung Box Test

# Columns & max lags set in chunk above
...
for (model_name in formula_cols) {
    ...
    for (h in 1:max_h){
        ...
        # max lag according to slides arround sqrt T
