...
# Plot
final_fc_plot2 <- ggplot(plot_df, aes(x = quarter_numeric, y = Value, group = 1)) +
  
  # Prediction interval (forecast only)
  geom_ribbon(aes(x = quarter_numeric, ymin = lower_2_sd, ymax = upper_2_sd), fill = "#C0C0C0", alpha = 0.5, data = forecast_df) +
  geom_ribbon(aes(x = quarter_numeric, ymin = lower_1_sd, ymax = upper_1_sd), fill = "#A9A9A9", alpha = 0.3, data = forecast_df) +
  
  # Line for actual + forecast
  geom_line(color = "#3A3A66", size = 1.2) +
  
  # X-axis formatting
  scale_x_continuous(
    breaks = plot_df$quarter_numeric,
    labels = plot_df$quarter_label
  ) +
