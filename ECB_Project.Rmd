...
$$
\begin{aligned}
  \forall h \text{:} \quad x_{t+h|t}^* &= \mathbb{E}_t [\phi_1^x x_{t+h-1} + \phi_2^x x_{t+h-2} + \phi_3^x x_{t+h-3} + \phi_4^x x_{t+h-4} + \epsilon_{t+h}^x + \theta_1^x \epsilon_{t+h-1} + \theta_2^x \epsilon_{t+h-2} + \theta_3^x \epsilon_{t+h-3} + \theta_4^x \epsilon_{t+h-4}] \\
...
\end{aligned}
$$
Where $x = \{ \tilde{\pi}_t , \tilde{y}_t , i_t \}$ (for the benchmark). We thus prove that our forecasts using the ARMA models are optimal for horizon 1. We then apply this using the Taylor Rule formula above to prove that this implies our model forecasts the interest rate optimally for horizon 1. 
As all the equations are linear, we can take advantage of the chain rule of forecasting for iterative forecasting for all horizons $h>1$, plugging in the values forecasted for $(t+1)$ in order to get forecasts for $(t+2)$ and so on, which is what we do until horizon 10.

\textcolor{red}{Note: something something note that we do not actually use arma(4,4) specifications. the actual specifications we use (insert those that are output in the end of chunks) where d=1 for inflation and interest rate as those are non-stationary}
