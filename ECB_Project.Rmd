...
Here we run the Ljung-Box test to test if the model contains a lack of fit, or in other words, if there is significant autocorrelation amongst the model errors. Rejecting H0 means therefore that the fit of the model is bad and we're missing some explainative Variable. Concrently the Test statistic is as follows:

$$
Q_{LB} = T(T+2) \sum_{\tau =1}^{m} (\frac{1}{1 - \tau}) \hat{p}^2(\tau) \xrightarrow[]{d} \chi^2_{m}
...
Run the Jarque-Bera test using a custom function that runs the test on all horizon errors and tests
if they are normally distributed. The test statistic uses the third and forth moment (Skewness and Kurtosis) and 
checks if the match a normal distribution. If H0 is rejected, they errors aren't normally distributed.
