---
title: "Forecasting the ECB Deposit Facility Rate - Coding Appendix"
author: 
- Jonas Bruno, Marcos Constantinou, Benoit Goye
output:
  pdf_document: 
    toc: true
    fig_caption: yes
...
* Shadow Interest Rate: The Shadow rate was developed by 
                        Wu & Xia (2017) and does quantify 
                        a hypothetical removal of the ZLB as it tracks the deposit
                        facility rate very closely before the ZLB.
...
computed with the HP filter and GDP output gap computed with the Hamilton filter. Note that we have no missing values, so we do not do any operation to deal with NA's. We do however cut out Q4 of 2025 as the data on said quarter is not fully available.
...
the results and interpret them based on the relevant p-value. For the ADF tests, a p-value smaller
than 0.05 means that we reject the null hypothesis H0 that the time series has a unit root (i.e non stationary). For the KPSS test, a p-value smaller than 0.05 means that we reject stationarity of the time series.
...
check_stationarity <- function(y, var_name) {
  ...
  if (adf_p < 0.05 && kpss_p > 0.05) {
    interpretation <- "Stationary I(0): ADF rejects unit root, 
                        KPSS confirms stationarity." 
  } else if (adf_p > 0.05 && kpss_p < 0.05) {
    interpretation <- "Non-Stationary I(1): ADF confirms unit root, 
                        KPSS rejects stationarity." 
  } else if (adf_p > 0.05 && kpss_p > 0.05) {
    interpretation <- "Conflicting Results: ADF confirms unit root, 
                        while KPSS suggests stationarity." 
  } else { # adf_p < 0.05 && kpss_p < 0.05
    interpretation <- "Conflicting Results: ADF rejects unit root, 
                        while KPSS rejects stationarity." 
  }
...
to get some information on the historical Taylor coefficients. We use Newey-West standard errors 
which is standard for time series estimation.
...
#Note that this helper function reports the Newey-West standard errors.
...
of the COVID-19 pandemic. The null hypothesis is that there are no structural breaks, and thus a p-value below 0.05 signifies we reject said hypothesis and believe that there is a break. The Bai-Perron test, however, works by finding any
structural breaks present in the overall sample which minimise
...
## Methodology
Our method relies on forecasting inputs to a Taylor Rule using ARIMA models, and then fitting those using the Taylor Rule coefficients to forecast the interest rate.


### Optimal Forecast Proof
We thus need to prove optimality of ARMA forecasts for the inflation gap, the output gap, and the interest rate (for the benchmark). We can then prove optimality of the Taylor Rule forecasts.

Inflation Gap:
$$
\begin{aligned}
    \tilde{\pi}_t &\equiv \pi_t - \pi^* \quad \overset{\mathrm{}}{\sim} \; ARMA(p,q) \\
    \tilde{\pi}_t &= \phi_1^\pi \tilde{\pi}_{t-1} + \phi_2^\pi \tilde{\pi}_{t-2} + \phi_3^\pi \tilde{\pi}_{t-3} + \phi_4^\pi \tilde{\pi}_{t-4} + \epsilon_t^\pi + \theta_1^\pi \epsilon_{t-1} + \theta_2^\pi \epsilon_{t-2} + \theta_3^\pi \epsilon_{t-3} + \theta_4^\pi \epsilon_{t-4}
\end{aligned}
$$
Where $\pi^*$ is the ECB's inflation target, and where we use $max(p) = 4 = max(q)$ since we use quarterly data.


Output Gap: 
$$
\begin{aligned}
    \tilde{y}_t &\equiv y_t - \bar{y}_t \quad \overset{\mathrm{}}{\sim} \; ARMA(p,q) \\
    \tilde{y}_t &= \phi_1^y \tilde{y}_{t-1} + \phi_2^y \tilde{y}_{t-2} + \phi_3^y \tilde{y}_{t-3} + \phi_4^y \tilde{y}_{t-4} + \epsilon_t^y + \theta_1^y \epsilon_{t-1} + \theta_2^y \epsilon_{t-2} + \theta_3^y \epsilon_{t-3} + \theta_4^y \epsilon_{t-4}
\end{aligned}
$$
Where $\bar{y}_t$ is potential output in , and where we use $max(p) = 4 = max(q)$ since we use quarterly data.


Interest Rate:
$$
\begin{cases}
  \text{ Taylor Rule:} \quad i_t = \pi^* + \phi^i i_{t-1} + \beta(\pi_t - \pi^*) + \gamma(y_t-\bar{y}_t) \\
  \text{ Benchmark:} \quad i_t \overset{\mathrm{}}{\sim} \; ARMA(p,q)
\end{cases}
$$
Where the Taylor Rule model is based on "Formula 3" and the benchmark follows some ARMA process.


Proof:
$$
\begin{aligned}
  \forall h \text{:} \quad x_{t+h|t}^* &= \mathbb{E}_t [\phi_1^x x_{t+h-1} + \phi_2^x x_{t+h-2} + \phi_3^x x_{t+h-3} + \phi_4^x x_{t+h-4} + \\ & \qquad \quad  \epsilon_{t+h}^x + \theta_1^x  \epsilon_{t+h-1} + \theta_2^x \epsilon_{t+h-2} + \theta_3^x \epsilon_{t+h-3} + \theta_4^x \epsilon_{t+h-4}] \\
  &= \mathbb{E}_t \left[ \sum_{i=1}^4 \phi_i^x x_{t+h-i} + \epsilon_{t+h} + \sum^4_{i=1}\theta^x_i \epsilon_{t+h-i} \right] \\
  \text{for } h=1 \text{:} \quad x_{t+1|t}^* &= \sum_{i=1}^4 \phi_i^x \mathbb{E}_t [  x_{t+1-i} ] +  \mathbb{E}_t [\epsilon_{t+1}] + \sum^4_{i=1}\theta^x_i \mathbb{E}_t [\epsilon_{t+1-i}] \\
  &= \phi^x_1x_t+\phi^x_2x_{t-1}+\phi^x_3x_{t-2}+\phi^x_4x_{t-3}+\theta^x_1\epsilon_{t}+\theta^x_2\epsilon_{t-1}+\theta^x_3\epsilon_{t-2}+\theta^x_4\epsilon_{t-3} \\
  \text{Which implies:} \quad i_{t+1|t}^* &= \mathbb{E}_t [i_{t+1}]  \\
              &= \mathbb{E}_t [\pi^* + \phi^i i_t + \beta \tilde{\pi}_{t+1} + \gamma \tilde{y}_{t+1} + \epsilon_{t+1}] \\
              &= \pi^* + \phi^i i_t + \beta \tilde{\pi}_{t+1|t}^* + \gamma \tilde{y}_{t+1|t}^*
\end{aligned}
$$
Where $x = \{ \tilde{\pi}_t , \tilde{y}_t , i_t \}$. 

We thus prove that our forecasts using the ARMA models are optimal for horizon 1. We then apply this using the Taylor Rule formula above to prove that this implies our model forecasts the interest rate optimally for horizon 1. As all the equations are linear, we can take advantage of the chain rule of forecasting for iterative forecasting for all horizons $h>1$, plugging in the values forecasted for $(t+1)$ in order to get forecasts for $(t+2)$ and so on, which is what we do until horizon 10.
Note that we do not actually use ARMA(4,4) specifications in the empirical application of this, but that this consists the upper bound of the number of $p$ and $q$ of our specifications.


### Empirical Application
The way we apply the theory consists of two steps. First, we run our custom auto 
ARIMA function on the inputs of the Taylor Rule (inflation and output gap) which automatically selects the best combination of $p$ and $q$ that minimises the AIC. This 
...
stationarity tests run above. Since the output gap is the only non-stationary variable, we set $d=0$ for that variable and $d=1$ for the others. The maximum $p$ and $q$ we use is four, as we have quarterly data. We additionally run this function on the interest
...
procedure which runs these loops in parallel on a fourth of the users' CPU cores.
...
forecast_to_df <- function(forecast_list, period) {
  ...
  numeric_list <- lapply(forecast_list, function(x) as.numeric(x)) #make each list numeric
...
```{r Density of Forecast Errors, warning=FALSE, message=FALSE}
...
### Variance of FE
```{r Variance of Forecast Errors, warning=FALSE, message=FALSE}
...
We utilize the Durbin-Watson test to detect the presence of first-order autocorrelation in the forecast errors at the $h=1$ horizon. The null hypothesis ($H_0$) assumes that the residuals are uncorrelated, while the alternative hypothesis ($H_a$) suggests that they follow a first-order autoregressive process. The test statistic $d$ is defined as the ratio of the sum of squared differences in successive residuals to the residual sum of squares:
...
Where $e_t$ represents the forecast error at time $t$. The statistic $d$ typically ranges between 0 and 4; a value near 2 indicates no serial correlation, whereas values approaching 0 suggest positive autocorrelation and values approaching 4 suggest negative autocorrelation. Below, we apply this test to the $h=1$ errors across all models to ensure that the immediate forecast residuals do not contain exploitable patterns ("can't forecast the errors"). A p-value below 0.1 indicates that we reject $H_0$ at the 10% level and a p-value of 0.05 and 0.01 mean we reject at a 5% and 1% level respectively.

We find that the models based on the Taylor Rule formula without lags have heavily autocorrelated errors significant at the 1% level, while the ones with lagged inflation exhibit autocorrelation to a lesser degree. For our model based on formula 3, we reject $H_0$ at only the 5\% significance level, while model 4 exhibits no autocorrelation in the horizon 1 errors.
...
We employ the Ljung-Box test to assess model adequacy by checking for significant autocorrelation in the residuals. The null hypothesis ($H_0$) assumes the errors are white noise; therefore, a rejection of $H_0$ indicates that the model fails to capture specific temporal dependencies, suggesting a potential lack of fit or model misspecification. The test statistic is specified in \eqref{eq:LB} where $T$ represents the sample size, $m$ is the number of lags being tested, and $\hat{\rho}(\tau)$ is the sample autocorrelation at lag $\tau$. Under $H_0$, $Q_{LB}$ asymptotically follows a $\chi^2$ distribution with $m$ degrees of freedom. Below we run the test for all models using the function from the stats package.

\begin{equation} 
\begin{aligned}
  Q_{LB} = T(T + 2) \sum_{\tau=1}^{m} \left( \frac{1}{T - \tau} \right) \hat{\rho}^2(\tau) \xrightarrow{d} \chi^2_m
\end{aligned}
\label{eq:LB}
\end{equation}
...
We reject the null hypothesis for the “no-lag” models (1 & 2), indicating that the residuals exhibit significant serial correlation at $h=1$. For these models, the errors can not be considered White Noise, suggesting that they may be misspecified or missing an explanatory variable. In contrast, for the lagged Taylor Rule models, we only reject the null for $h=1$§ at the 5% level for model 3 and for model 4 we do not reject the null at $h=1$ at all. We conclude from these results that model 1 and 2 have heavy autocorrelation in the $h=1$ residuals, while model 3 has autocorrelation to a lesser degree, and model 4 has none at all.

Since we utilize an iterative forecasting approach where the $h$-step ahead forecast is contingent upon the $h=1$ prediction, any misspecification identified by the Ljung-Box test at short horizons is subject to propagation across the entire forecast trajectory. 


### Errors Normally Distributed
We Run the Jarque-Bera test using a function from the tseries package that runs the test on all horizon errors and tests if they are normally distributed. The test is specified in \eqref{eq:JB}, where $n$ is the number of observations, $S$ is the sample skewness, and $K$ is the sample kurtosis. Under $H_0$, the statistic asymptotically follows a $\chi^2$ distribution with 2 degrees of freedom. A rejection of $H_0$ indicates that the residuals exhibit significant asymmetry (or "fat tails"), suggesting that standard confidence intervals may be inappropriate.
...
The test statistic is as reported in equation \eqref{eq:DM} and 
assumes that $\hat{d}_{h,t}$ is covariance stationary. We include three versions
...
\begin{aligned}
  DM &= \frac{\bar{\hat{d_h}}}{\hat{\sigma^2_{\bar{\hat{d_h}}}}} \\
   ...
   \text{where } \hat{d}_{h,t} &= \hat{e}_{1,t+h|t}^2 - \hat{e}_{2,t+h|t}^2
\end{aligned}
\label{eq:DM}
\end{equation}
