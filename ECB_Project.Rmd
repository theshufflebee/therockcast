...
We Run the Jarque-Bera test using a function from the tseries package that runs the test on all horizon errors and tests if they are normally distributed. The Test is specified in \eqref{eqJB}, where $n$ is the number of observations, $S$ is the sample skewness, and $K$ is the sample kurtosis. Under $H_0$, the statistic asymptotically follows a $\chi^2$ distribution with 2 degrees of freedom. A rejection of $H_0$ indicates that the residuals exhibit significant asymmetry (or "fat tails)," suggesting that standard confidence intervals may be inappropriate.
